...
MT2TreeBkg::MT2TreeBkg(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("softLep-v3_V00-01-10_25ns_miniaodv2/ttsl_mg_lo_top.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("softLep-v3_V00-01-10_25ns_miniaodv2/ttsl_mg_lo_top.root");
      }
...
MT2TreeBkg::~MT2TreeBkg()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t MT2TreeBkg::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MT2TreeBkg::LoadTree(Long64_t entry)
{
...
void MT2TreeBkg::Init(TTree *tree)
{
...
void MT2TreeBkg::Show(Long64_t entry)
{
...
Int_t MT2TreeBkg::Cut(Long64_t entry)
{
