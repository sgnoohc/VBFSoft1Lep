...
if __name__ == "__main__":
    ...
    for branch in branches:
        ...
        if not cname:
            ...
            ## if the btitle contains "[blah]" it means that this branch is an array
            #if len(btitle.split("[")) > 1:
            #    cname += "*"

        typ = cname[:]

        # parse the size of the array
        print bname, cname, btitle
        size = "0"
        if len(btitle.split("[")) > 1:
            print btitle
            size = btitle.split("[")[1].split("]")[0]
    ...
    for bname in d_bname_to_info:
        ...
        size = d_bname_to_info[bname]["size"]
        if size != "0":
            #buff += '  %s %s_[%s_];\n' % (typ.replace("const","").strip(), alias, size)
